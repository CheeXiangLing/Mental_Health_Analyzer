...
# === Improved Download Function with Progress ===
@st.cache_resource
...
def verify_model(model_name):
    folder = f"{model_name.lower()}_model"
    required_files = {
        "BERT": ["config.json", "pytorch_model.bin"],
        "DistilBERT": ["config.json", "pytorch_model.bin"],
        "RoBERTa": ["config.json", "pytorch_model.bin"]
    }
    
    if not os.path.exists(folder):
        return False
        
    for file in required_files.get(model_name, []):
...
def predict_transformer(model_name, text):
    try:
        ...
        if not verify_model(model_name):
            if not download_and_extract_model(model_name):
                return "Model Error"
...
with st.sidebar:
    ...
    *For research/educational purposes only.*
    ...
    Transformers: {BertTokenizerFast.__version__}
    """)
